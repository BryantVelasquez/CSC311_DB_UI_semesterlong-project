...
public class DbConnectivityClass {
    ...
    final static String SQL_SERVER_URL = "jdbc:mysql://velasquezcsc311server.mysql.database.azure.com/";//update this server name
    final static String DB_URL = "jdbc:mysql://velasquezcsc311server.mysql.database.azure.com/" + DB_NAME;//update this database name
    final static String USERNAME = "Velasquez";// update this username
    final static String PASSWORD = "Dp215980519";// update this password
    ...
    public boolean loginStuff(String userName, String passWord) {
        connectToDatabase();
        try {
            Connection conn = DriverManager.getConnection(DB_URL, USERNAME, PASSWORD);
            String sql = "SELECT * FROM Scotts WHERE username=? AND password=?";
            PreparedStatement preparedStatement = conn.prepareStatement(sql);
    ...
    public boolean createStuff(String userName, String passWord) {
        try {
            Class.forName("com.mysql.cj.jdbc.Driver");
            Connection conn = DriverManager.getConnection(DB_URL, USERNAME, PASSWORD);
            Statement statement = conn.createStatement();
            String whatever = "INSERT INTO Scotts(username,password) VALUES(?,?)";
            String sql = "CREATE TABLE IF NOT EXISTS Scotts(username VARCHAR(200) NOT NULL PRIMARY KEY, password VARCHAR(200) NOT NULL)";
            statement.executeUpdate(sql);
            statement.close();
            PreparedStatement preparedStatement = conn.prepareStatement(whatever);
            preparedStatement.setString(1, userName);
            preparedStatement.setString(2, passWord);
            int whatever2 = preparedStatement.executeUpdate();
            preparedStatement.close();
            conn.close();
            if(whatever2 <= 0){
                return false;
